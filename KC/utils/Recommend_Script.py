...
class Product:
    ...
    # 获取评分表
    def update_gradeForm(self, uid, pid, grade):
        uid2 = int(get2id(uid))
        # 新建列
        if not uid2 in self.gradeForm.columns:
            temp = np.zeros(self.gradeForm.shape[0])
            self.gradeForm.insert(1, uid2, temp)
        # 新建行
        if not pid in self.gradeForm.index:
            temp = DataFrame(np.zeros(self.gradeForm.shape[0]))
            concat([self.gradeForm, temp], ignore_index=True)
        self.gradeForm.loc[pid][uid2] = grade
...
class Pro_rec(Product):
    ...
    def get_user_dailyRec(self, uid):
        rec_list = self.get_dailyRec()
        id = get2id(uid)
        if id in rec_list:
            rec_list = rec_list[id]
        else:
            rec_list = self.productRank
        return rec_list
    ...
    def get_timelyRec(self, uid, k=1):
        ...
        if uid in ratings.index:
            ...
            if end > len(sim_user_list) - 1 or len(sim_user_list) == 0:
            ...
            # 将推荐列表中推荐用户已评分产品去除
            rec_list = [i for i in rec.index if i not in set(uid_list.index)] if len(uid_list) > 0 else rec
            return rec_list
        else:
            return self.productRank
