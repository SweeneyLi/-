...
def add_user_grade():
    ...
    if pid and uid and grade:
        ...
        if tp == "股票":
            pro_rec.update_gradeForm(uid, pid, grade)

            gg = Grade.query.filter(db.and_(Grade.uid == uid, Grade.pid == pid)).first()
            if gg:
                gg.grade = grade
                print(gg)
            else:
                gg = Grade(uid=uid, pid=pid, grade=grade)

        elif tp == "债券":
            pro_rec_1.update_gradeForm(uid, pid, grade)
            gg = Grade_1.query.filter(db.and_(Grade_1.uid == uid, Grade_1.pid == pid)).first()
            if gg:
                gg.grade = grade
                print(gg)
            else:
                gg = Grade_1(uid=uid, pid=pid, grade=grade)
        else:
            pro_rec_2.update_gradeForm(uid, pid, grade)
            gg = Grade_2.query.filter(db.and_(Grade_2.uid == uid, Grade_2.pid == pid)).first()
            if gg:
                gg.grade = grade
                print(gg)
            else:
                gg = Grade_2(uid=uid, pid=pid, grade=grade)
...
def random_rec():
    ...
    if tp == "股票":
        ...
        # products = random.choices(products, k=1)
        # infos = [[product.symbol, product.name] for product in products]

        product_id = random.choices(products, k=1)[0].id
        ...
        products = Product_2.query.filter(
            Product_2.symbol.notin_([userproduct.pid for userproduct in userproducts])).all()
        # products = random.choices(products, k=1)
        # infos = [[product.symbol, product.name] for product in products]
